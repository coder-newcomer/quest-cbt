<div align="center">

# <font size="7" align="center">📜 Quest CBT</font>

**An Electron application for creating and running Computer Based Test (CBT) on local network.**

[![GitHub License](https://img.shields.io/github/license/coder-newcomer/quest-cbt?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%3D%3D)](LICENSE)
[![electron-vite](https://img.shields.io/badge/Scaffolded_with-electron--vite-6988e6.svg?style=for-the-badge&logo=data:image/svg+xml;base64,&logoColor=white)](https://electron-vite.org)
[![SolidJS](https://img.shields.io/badge/SolidJS-446b9e?style=for-the-badge&logo=solid&logoColor=c8c9cb)](https://www.solidjs.com)

<!-- Project Badges -->

![GitHub package.json version](https://img.shields.io/github/package-json/v/coder-newcomer/quest-cbt?logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BPC9zdmc%2B)
![GitHub Release Date](https://img.shields.io/github/release-date/coder-newcomer/quest-cbt?logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BPC9zdmc%2B)
![GitHub commit activity](https://img.shields.io/github/commit-activity/w/coder-newcomer/quest-cbt?logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BPC9zdmc%2B)
[![GitHub last commit](https://img.shields.io/github/last-commit/coder-newcomer/quest-cbt?logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%3D)](commits)
[![GitHub contributors](https://img.shields.io/github/contributors/coder-newcomer/quest-cbt?style=flat&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%3D)](graphs/contributors)

![screenshot](./docs/images/screenshot.png)

![Built by Developers](https://forthebadge.com/images/badges/built-by-developers.svg 'Yet, only me the only developer')
![Built with Love](https://forthebadge.com/images/badges/built-with-love.svg)
![0% Optimized](https://forthebadge.com/images/badges/0-percent-optimized.svg 'Early development please')

</div>

---

<br/>

## ℹ️ About

The name was inspired by quest/mission system on RPG games. As being said, you can carry out any test as "quest" that are compatible with the application itself — as being made with Electron, so you can develop them with HTML5 and integrate some functionality with its own derived API.

## 🚩 Getting Started

<details>
<summary>

### ❇️ <!--📑--> Prerequisites</summary>

> [!NOTE] TODO
> What are the project requirements/dependencies?

</details>
<details>
<summary>

### 💽 Installation</summary>

> [!NOTE] TODO
> Describe how to install and get started with the project.

</details>
<details>
<summary>

### 👤 Usage</summary>

> [!NOTE] TODO
> How does one go about using it?
> Provide various use cases and code examples here.

</details>

## 🗺️ Roadmap

See the [open issues](issues) for a list of proposed features and known issues.

- [Top Feature Requests](issues?q=label%3Aenhancement+is%3Aopen+sort%3Areactions-%2B1-desc) (Add your votes with <kbd>👍</kbd>)
- [Top Bugs](issues?q=is%3Aissue+is%3Aopen+label%3Abug+sort%3Areactions-%2B1-desc) (Add your votes with <kbd>👍</kbd>)
- [Newest Bugs](issues?q=is%3Aopen+is%3Aissue+label%3Abug)
- [TODO](TODO.md) list

## 💬 Support

This project is still on early stage of development. Contributions and PRs are very welcome. See for further information about [contributing](docs/CODE_OF_CONDUCT.md).

If you have any questions, please feel free to [open an issue](issues/new/choose). Or reach out maintainers at [GitHub issues](issues/new?assignees=&labels=question&template=04_SUPPORT_QUESTION.md&title=support%3A+), [contributors](graphs/contributors), or the following contacts:

[![Discord](https://img.shields.io/badge/coder--newcomer-DMs-7289da.svg?&logo=discord&logoColor=white)](https://discord.gg/Hqz7vyK9)
<!-- [![Discord](https://img.shields.io/badge/%E2%80%8B-Contact_me-7289da.svg?&logo=discord&logoColor=white)](https://discord.gg/Hqz7vyK9) -->
